...
# Get APPDATA directory and create a dedicated subscriptions folder
APPDATA_DIR = os.getenv("APPDATA")
SUBSCRIPTIONS_DIR = os.path.join(APPDATA_DIR, "MyAppSubscriptions")
os.makedirs(SUBSCRIPTIONS_DIR, exist_ok=True)  # Ensure the directory exists

# File to store user info (email and phone) permanently
USER_INFO_FILE = os.path.join(SUBSCRIPTIONS_DIR, "user_info.json")

# The secret salt must match the one used for key generation
SECRET_SALT = "01apr2025"

# The strategy identifier ensures that the subscription key is unique to this file/strategy.
# For example, use "strategyA" for one file and "strategyB" for another.
STRATEGY_ID = "EMA"
CONFIG_FILE = os.path.join(SUBSCRIPTIONS_DIR, f"{STRATEGY_ID}.json")

# Subscription Durations (days) using shorthand as per key generation
...
def prompt_and_store_user_info():
    """Prompt the user to enter their email and phone, then store them permanently."""
    root = tk.Tk()
    root.withdraw()
    email = simpledialog.askstring("User Info Required", "Enter your email:")
    phone = simpledialog.askstring("User Info Required", "Enter your phone number:")
    if not email or not phone:
        ctypes.windll.user32.MessageBoxW(0, "Email and phone are required. Exiting...", "Error", 0x10)
        exit()
    user_info = {"email": email.strip(), "phone": phone.strip()}
    with open(USER_INFO_FILE, "w") as f:
        json.dump(user_info, f)
    # Hide the file using Windows attrib command
    os.system(f'attrib +h "{USER_INFO_FILE}"')
    return user_info

def get_user_info():
    """Load stored user info; if not found, prompt the user to enter it."""
    if os.path.exists(USER_INFO_FILE):
        try:
            with open(USER_INFO_FILE, "r") as f:
                return json.load(f)
        except Exception:
            return prompt_and_store_user_info()
    else:
        return prompt_and_store_user_info()

# Load user info (this will be used for validation and key generation)
user_info = get_user_info()
# Now, these values are fixed and cannot be changed by the user.
REGISTERED_EMAIL = user_info["email"]
REGISTERED_PHONE = user_info["phone"]

def compute_expected_key(email, phone, strategy, duration):
    """
    Recomputes the subscription key base given the registered email, phone, strategy identifier,
    duration, and secret salt. This should match the first part of the key generated by your key
    generation script.
    """
    raw_string = f"{email}:{phone}:{strategy}:{duration}:{SECRET_SALT}"
    hashed = hashlib.sha256(raw_string.encode()).hexdigest()
    key = base64.urlsafe_b64encode(hashed.encode()).decode()[:16]
    return key

def get_chrome_email():
    """
    Fetch the Gmail account used in Chrome.
    Looks for the Chrome Preferences file and tries to extract the email.
    """
    local_appdata = os.getenv("LOCALAPPDATA")
    chrome_path = os.path.join(local_appdata, "Google", "Chrome", "User Data", "Default", "Preferences")
    if os.path.exists(chrome_path):
        try:
            with open(chrome_path, "r", encoding="utf-8") as f:
                prefs = json.load(f)
                if "account_info" in prefs and prefs["account_info"]:
                    first_account = prefs["account_info"][0]
                    if "email" in first_account:
                        return first_account["email"]
        except Exception:
            return None
    return None

def get_windows_linked_email():
    """
    Fetch the Windows-linked email (non-Gmail).
    Uses the 'whoami /upn' command to try to get the user principal name.
    """
    try:
        email = os.popen("whoami /upn").read().strip()
...
def validate_user():
    """
    Check if the EXE is being run by the registered user.
    It checks if either the Gmail from Chrome OR the Windows-linked email matches the stored email.
    """
    system_email = get_chrome_email() or get_windows_linked_email()
    if system_email and system_email.lower() == REGISTERED_EMAIL.lower():
        return True
    ctypes.windll.user32.MessageBoxW(0, "Unauthorized email detected!", "Access Denied", 0x10)
    exit()

def save_subscription_data(data):
    """Save trial or subscription details to a file and mark it as hidden."""
    with open(CONFIG_FILE, "w") as f:
        json.dump(data, f)
    os.system(f'attrib +h "{CONFIG_FILE}"')

def load_subscription_data():
    """Load subscription details from the file."""
...
def check_subscription():
    """
    Check if the user has an active subscription or is in trial mode.
    If neither is valid, show the subscription GUI.
    """
    ...
    if "expiry_date" in data:
        ...
        if today <= expiry_date:
            return True  # Subscription or trial is active
    activate_trial_or_subscription()
    exit()

def activate_trial_or_subscription():
    """Offer trial activation if available, otherwise ask for a subscription key."""
    data = load_subscription_data()
    if "trial_used" not in data or not data["trial_used"]:
        expiry_date = datetime.date.today() + datetime.timedelta(days=30)
        save_subscription_data({"trial_used": True, "expiry_date": expiry_date.strftime("%Y-%m-%d")})
        ctypes.windll.user32.MessageBoxW(0, "Free trial activated!", "Success", 0x40)
        return
    show_subscription_window()
    exit()

def activate_subscription(full_key):
    """
    Activate subscription based on a valid subscription key.
    The key is expected to be in the format:
       <generated_key>-<duration>
    For example: "eK4mHf8s9PqLrT3X-3M"
    """
    try:
        base_key, duration = full_key.rsplit("-", 1)
    except ValueError:
        ctypes.windll.user32.MessageBoxW(0, "Invalid subscription key format!", "Error", 0x10)
        exit()
    if duration not in SUBSCRIPTION_DURATIONS:
        ctypes.windll.user32.MessageBoxW(0, "Invalid subscription duration in key!", "Error", 0x10)
        exit()
    expected_key = compute_expected_key(REGISTERED_EMAIL, REGISTERED_PHONE, STRATEGY_ID, duration)
    if base_key != expected_key:
        ctypes.windll.user32.MessageBoxW(0, "Invalid subscription key!", "Error", 0x10)
        exit()
    expiry_date = datetime.date.today() + datetime.timedelta(days=SUBSCRIPTION_DURATIONS[duration])
    save_subscription_data({"expiry_date": expiry_date.strftime("%Y-%m-%d"), "trial_used": True})
    ctypes.windll.user32.MessageBoxW(0, "Subscription activated!", "Success", 0x40)

def show_subscription_window():
    """
    Show a GUI window for the user to enter their subscription key.
    """
    root = tk.Tk()
    root.withdraw()
    full_key = simpledialog.askstring("Subscription Required", "Enter your subscription key:")
    if full_key:
        activate_subscription(full_key)
    else:
        ctypes.windll.user32.MessageBoxW(0, "No key entered. Exiting...", "Error", 0x10)
        exit()

# Run checks before execution
validate_user()
check_subscription()

# If the code reaches here, either a valid trial or subscription is active.
print("Access granted. Running software...")

# Place your main software logic below this line.
